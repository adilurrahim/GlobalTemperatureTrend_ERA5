...
years = [i for i in range(1987,2021)]
...
for year in years:
    ...
    for month in months:
        ...
        df2.to_csv(r'C:\Users\mrahim6\OneDrive - Louisiana State University\Papers\Temperature trend\Paper 1\Mann-Kendall\Analysis\Temperatures'+'/average/'+mnt[5:7]+'/'+str(year)+'.csv',index=False)        
    ...
    for month in months:
        ...
        df2['MeanTemp'] = df.mean(axis=1)
        df2.to_csv(r'C:\Users\mrahim6\OneDrive - Louisiana State University\Papers\Temperature trend\Paper 1\Mann-Kendall\Analysis\Temperatures'+'/mean/'+mnt[5:7]+'/'+str(year)+'.csv',index=False)        
...
for mnt in months:
    year = years[0]
    df_avg = pd.read_csv(path+'/average/'+mnt+'/'+str(year)+'.csv')
    df2 = df_avg[['MeanTemp']]
    for year in years[1:]:
    ...
    year = years[0]
    df_mean = pd.read_csv(path+'/mean/'+mnt+'/'+str(year)+'.csv')
    ...
    df3.to_csv(path+'/Difference_1981_2020_'+mnt+'.csv',index=False)
#####################################
path = r'C:\Users\mrahim6\OneDrive - Louisiana State University\Papers\Temperature trend\Paper 1\Mann-Kendall\Analysis\Temperatures'
months = ['01','02','03','04','05','06','07','08','09','10','11','12']
...
for mnt in months:
    ...
    df3.to_csv(path+'/average/'+mnt+'/TempVariation'+'.csv')
    ####
    years = [i for i in range(2011,2021)]
    year = years[0]
    df_avg = pd.read_csv(path+'/mean/'+mnt+'/'+str(year)+'.csv')
    df2 = df_avg[['MeanTemp']]
    for year in years[1:]:
    ...
    df4 = df_1981_1990[['x','y']]
    df4['TempDiff'] = df_2011_2020['MeanTemp'] - df_1981_1990['MeanTemp']
    
    df4.to_csv(path+'/mean/'+mnt+'/TempVariation'+'.csv')
...
df_1981_2020_average = pd.read_csv(path+'/'+'1981-2020.csv')
...
df_1981_2020_mean = pd.read_csv(path+'/'+'1981-2020.csv')


df1 = df_1981_2020_average[['x','y']]
df1['TempDiff'] = df_1981_2020_mean['MeanTemp'] - df_1981_2020_average['MeanTemp']
df1.to_csv(path+'/Difference_1981_2020.csv',index=False)

list1.append([max(df1['TempDiff']), min(df1['TempDiff']), latitude_mean(df1)])
...
df1['TempDiff'] = df_2011_2020['MeanTemp'] - df['MeanTemp']
